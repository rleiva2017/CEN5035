...
public class Gameplay extends JPanel implements KeyListener, ActionListener{
	...
	public void paint(Graphics g) {
		...
		if(totalBricks <= 0)
		{
			...
			g.drawString("Press Space bar to Continue", 230, 350);
		}
		...
		if(ballposY>570)
		{
			...
			g.setFont(new Font("serif",Font.BOLD,20));
			g.drawString("Press S to Save Score", 270, 400);
			if(saved == true)
			{
				...
				final ScoreDAO s =new ScoreDAO(conn);
				try {
					s.insert(user, score);
				} catch (Exception e) {
					System.out.println("Score did not send");
					e.printStackTrace();
				}
				g.setFont(new Font("serif",Font.BOLD,20));
				g.drawString("Save completed", 270, 450);
			}
